...
def post_advance_time(payload: AdvanceTimeRequest, db=Depends(get_db), current_user=Depends(get_current_user)):
...
def post_reset_time(db=Depends(get_db), current_user=Depends(get_current_user)):
    reset_time(db)
    return TimeStatus(offset_seconds=get_offset_seconds(db), virtual_now=now_utc(db))
