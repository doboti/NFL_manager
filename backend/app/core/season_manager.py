...
def end_season(db: Session, league: League, now: datetime, champion_id: int) -> dict:
    ...
    # Fixtures are otherwise only (re)generated by the daily cycle -- without
    # calling this here, a league sits with zero scheduled matches (every
    # team's "next match" falling back to a rough, clock-mismatched guess)
    # until the next tick happens to run. Every team just got treated as
    # ownerless/fresh above, so this naturally schedules everyone's season
    # opener together, exactly like a real Week 1.
